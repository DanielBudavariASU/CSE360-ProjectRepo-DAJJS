...
public class InstructorGroupPage extends Application {
    ...
    class CardPane extends StackPane {
        ...
        private VBox InstructorPanel() {
            ...
            // Buttons
            Button viewButton = new Button("List Special Access Groups");
        ...
        private VBox createGroup() {
            ...
            createSpecialGroupButton.setOnAction(e -> {
            	...
            	if(db.findSpecialAccessGroupByName((specialgroup)) == null)
            	{
                    ...
                    instructor.createGroup(db, newGroup);
                    db.addInstructorAdminToSpecialAccessGroup(newGroup.getGroupName(), instructor);
            		Alert successAlert = new Alert(Alert.AlertType.INFORMATION, "Instructor added as admin to Special Access Group!");
        ...
        private VBox viewMemberAccess() {
            ...
            Button listButton = new Button("List Special Access Groups");
            Label inaccessibleLabel = new Label("Instructor does not have access to these groups: ");

            // VBox to display the list of group members
            VBox articleDisplay = new VBox(5);
            VBox inaccessibleDisplay = new VBox(5);
            articleDisplay.setStyle("-fx-padding: 10; -fx-background-color: #f9f9f9; -fx-border-color: #ccc;");
            inaccessibleDisplay.setStyle("-fx-padding: 10; -fx-background-color: #f9f9f9; -fx-border-color: #ccc;");
            ...
            listButton.setOnAction(e -> {
                ...
                for (SpecialAccessGroup specialGroup : groups) {
                    ...
                    Label groupLabel = new Label(specialGroup.getGroupName());
                    VBox articleBox = new VBox(3, groupLabel);
                    articleBox.setStyle("-fx-padding: 5; -fx-background-color: #e6e6e6; -fx-border-color: #ccc;");

                    articleDisplay.getChildren().add(articleBox);
                }
               
                inaccessibleDisplay.getChildren().clear();
                ...
                if (inaccessibleGroups != null) {
                    ...
                    for (String inaccessibleGroupName : inaccessibleGroups) {
                        ...
                        VBox inaccessibleBox = new VBox(3, groupLabel);
                        inaccessibleBox.setStyle("-fx-padding: 5; -fx-background-color: #e6e6e6; -fx-border-color: #ccc;");
                        System.out.println(inaccessibleGroupName);
                        inaccessibleDisplay.getChildren().add(inaccessibleBox);
                    }
                }
            });

            panel.getChildren().addAll(returnButton, listButton, articleDisplay, inaccessibleLabel, inaccessibleDisplay);
            return panel;
        }
