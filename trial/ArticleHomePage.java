    ...
    class CardPane extends StackPane {
        ...
        private VBox createArticle() {
            ...
            panel.getChildren().addAll(
                ...
                new Label("General Groups:"), groupsField,
                new Label("Special Access Groups:"), specialGroupField,
                submitButton
            );
            ...
            submitButton.setOnAction(e -> {
                ...
                List<String> specialGroups = List.of(specialGroupField.getText().split("\\s*,\\s*"));
                List<String> specialGroupsToAdd = new ArrayList<>();
                List<String> groupToAdd = new ArrayList<>();

                boolean isSpecialAccess = false;
                boolean isInGeneralAccessGroup = false;
                ...
                if (!groups.isEmpty()) {
                    for (String group : groups) {
                        ...
                        if (db.findGroupByName(group) == null) {
                        	Alert alert = new Alert(Alert.AlertType.ERROR, "Help Group does not exist yet!");
                            alert.showAndWait();
                        } else {
                ...
                if(!specialGroups.isEmpty()) {
                	for (String specialgroup : specialGroups) {
                        ...
                        if (db.findSpecialAccessGroupByName(specialgroup) == null) {
                        	Alert alert = new Alert(Alert.AlertType.ERROR, "Special Access Group does not exist yet!");
                            alert.showAndWait();
                        } else {
                        	specialGroupsToAdd.add(specialgroup);
                        }
                    }
                }
        ...
        private VBox listArticles() {
            ...
            listButton.setOnAction(e -> {
                ...
                if(admin != null) {
                	articles = db.listHelpArticles(groupNames, admin);	
                }
                else {
                	articles = db.listHelpArticles(groupNames, instructor);
                }
                ...
                if (articles.isEmpty()) {
                    ...
                    for (HelpArticle article : articles) {
                        ...
                        Label specialGroupsLabel = new Label("Special Access Groups: " + article.getSpecialAccessGroups());
                        VBox articleBox = new VBox(3, idLabel, groupsLabel, specialGroupsLabel);
