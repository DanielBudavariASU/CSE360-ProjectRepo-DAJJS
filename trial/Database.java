...
public class Database {
    ...
    public boolean removeUser(User user) {
        if(user.isAdmin() == true && (users.getLast() != user)) {
    ...
    public void removeStudent(Student student) {
    	System.out.println("(db.removeStudent) parameter: "+ student.getUsername()); 
    	for(Student testStudent : students)
        {
        	System.out.println("students before removal (db.removeStudent)" + testStudent.getUsername());
        }
    	
    	for(User testUser : users)
        {
        	System.out.println("users before removal (db.removeStudent)" + testUser.getUsername());
        }
        ...
        if (students.contains(student) && users.contains(student)) {
            ...
            for(Student testStudent : students)
            {
            	System.out.println("students after removal (db.removeStudent)" + testStudent.getUsername());
            }
            ...
            for(User testUser : users)
            {
            	System.out.println("users after removal (db.removeStudent)" + testUser.getUsername());
            }
            ...
            for (SpecialAccessGroup group : specialAccessGroups) {
                if (group.getStudents().contains(student)) {
                    ...
                    System.out.println("Student removed successfully from the special access groups.");
                }
            }
    ...
    public Student findStudentByUsername(String username) {
        ...
        for (Student student : students) {
        	System.out.println("Student database: " + student.getUsername());
     ...
     */
    public boolean addSpecialAccessGroup(SpecialAccessGroup group) {
     ...
     */
    public boolean removeAdminFromSpecialAccessGroup(String groupName, Admin admin) {
        SpecialAccessGroup group = findSpecialAccessGroupByName(groupName);
        if (group != null) {
    ...
    public void removeInstructorAdminFromSpecialAccessGroup(String groupName, Instructor instructor) {
    	System.out.println("Database groupname: " + groupName);
        SpecialAccessGroup group = findSpecialAccessGroupByName(groupName);
        if (group != null) {
    ...
    public void removeStudentFromSpecialAccessGroup(String groupName, Student student) {
        ...
        if (group != null) {
            ...
            if (group.getStudents().contains(student.getUsername())) {
                group.removeStudent(student);
...
public List<HelpArticle> restoreHelpArticles(String fileName, boolean merge, User user) {
    ...
    public boolean hasAccessToArticle(User user, HelpArticle article) {
        if (article.isSpecialAccess()) {
        	...
        	for (String specialGroupName : article.getSpecialAccessGroups()) {
                System.out.println(specialGroupName);
        		SpecialAccessGroup specialGroup = findSpecialAccessGroupByName(specialGroupName);
    ...
    public HelpArticle returnArticletoDisplay(long articleId) {
    	HelpArticle article = findHelpArticleById(articleId);
    	HelpArticle articleToDisplay;
    	articleToDisplay = new HelpArticle(article);
    	if (article == null) {
