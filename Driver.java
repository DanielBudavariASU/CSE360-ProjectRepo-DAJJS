...
//newly added code below ***** phase 2
         
         /**
          * Creates the backup options page where the admin can enter group names for backup.
          * @return VBox containing the backup page UI
          */
         private VBox createBackupPage() {
             ...
             submitButton.setOnAction(e -> {
                 ...
                 confirmAlert.showAndWait().ifPresent(response -> {
                     if (response == ButtonType.OK) {
                         // Trigger the backup function with the specified group name
                         db.backupByGroup(groupName.isEmpty() ? null : groupName);
                         Alert successAlert = new Alert(Alert.AlertType.INFORMATION, "Backup successful!");
                         successAlert.showAndWait();
                         getChildren().clear();
                         getChildren().add(createAdminHomePage()); // Go back to home page
                     } else {
                         getChildren().clear();
                         getChildren().add(createAdminHomePage()); // Go back to home page
                     }
             ...
             panel.getChildren().addAll(instructions, groupNameField, submitButton);
             return panel;
         }

         /**
          * Creates the restore options page where the admin can choose between merge or overwrite.
          * @return VBox containing the restore page UI
          */
         private VBox createRestorePage() {
             ...
             submitButton.setOnAction(e -> {
                 ...
                 if (selectedOption != null) {
                     ...
                     confirmAlert.showAndWait().ifPresent(response -> {
                         if (response == ButtonType.OK) {
                             // Trigger the restore function with the chosen option
                             db.restore(option.equals("Overwrite"));
             ...
             panel.getChildren().addAll(instructions, mergeButton, overwriteButton, submitButton);
             return panel;
         }
// newly added code above ***** phase 2
    }
